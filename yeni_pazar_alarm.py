...
Alarm listesi ve ID haritasi GitHub Gist'ten otomatik yuklenir.
Eski pazar scriptiyle (ar_alarm.py) FARKLI bir Termux'ta calistir.
"""
...
# -- GitHub Gist (alarm + id haritasi) -------
GIST_ID    = "b6cae757f7651b69b99cb25b23bbf683"
GIST_FILE  = "ar_alarm.json"
# ---------------------------------------------

# =============================================
#  ALARM LISTESİ — GitHub Gist'ten yuklenir
# =============================================
ALARM_LIST = []   # baslangicta bos, gist'ten doldurulur
ID_MAP     = {}   # item_id -> item adi
...
def load_gist_config():
    ...
    try:
        ...
        for key, id_data in saved_ids.items():
            ...
            if not confirmed_ids: continue
            price_val, active = "", False
            try:
                price_val = prices.get(cat_id, {}).get(item_name, {}).get(level_name, {}).get("price", "")
                active    = prices.get(cat_id, {}).get(item_name, {}).get(level_name, {}).get("active", False)
            except: pass
            ...
            for id_ in confirmed_ids:
                ID_MAP[id_] = f"{item_name} {level_name}"
    ...
    try:
        ...
        for line in new_code.splitlines():
            if line.startswith("VERSION"):
                ...
                if new_ver != VERSION:
                    log(f"  Yeni versiyon: {new_ver} (mevcut: {VERSION}). Guncelleniyor...")
...
def fire_alarm(item_name, item_id, price, max_price):
    ...
    log(f"  *** ALARM *** {item_name}  |  ID:{item_id}  |  {price:,} gold")
    msg = (
...
def main():
    ...
    log("")
    ok = load_gist_config()
    if not ok or not ALARM_LIST:
        log("HATA: Alarm listesi yuklenemedi veya bos!")
        log("  Cozum: HTML'den bir itemi aktifle ve 'Ust Pazar Alarm Indir' tusuna bas.")
        return
    send_telegram(f"NOWA Ust Pazar Alarm baslatildi (v{VERSION}). {len(ALARM_LIST)} alarm aktif.")
    log("")
    ...
    try:
        while True:
            ...
            if not local_pcap:
                log("  Pcap alinamadi.")
                in_burst, burst_end_cnt = False, 0
                continue
            ...
            if len(payload) == 0:
                log("  Sunucu verisi bos.")
            else:
                recs = parse_yeni_pazar(payload)
                check_alarms(recs)

            log("  30sn sonra ust pazar persomenini tekrar ac.")
            log("")
            run_shell("su -c 'killall tcpdump 2>/dev/null'")
            time.sleep(2)
            tcpdump_proc = None
